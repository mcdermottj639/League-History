...
test('directory absent for ordinary members and does not duplicate public sharing',()=>{
 const regular=setup(false);assert.equal(regular.host.innerHTML,'');regular.dom.window.close();
 const h=setup(true);assert.equal(h.host.querySelectorAll('.lg-link-row').length,0);assert.doesNotMatch(h.host.textContent,/League app · public/);h.dom.window.close();
});
test('private rows render safely, persist locally, reject foreign links and recheck owner access',()=>{
 ...
 assert.equal(h.host.querySelector('img'),null);assert.match(h.host.textContent,/<img/);assert.equal(h.host.querySelectorAll('.lg-link-row button')[0].disabled,true);
 ...
 const select=h.host.querySelector('.lg-link-row select');select.value='ready';select.dispatchEvent(new h.w.Event('change'));assert.equal(h.host.querySelectorAll('.lg-link-row button')[0].disabled,false);
...
test('private setup saves the original organizer link ready for owner, strips URL and preserves other rows',()=>{
 ...
 assert.match(h.host.textContent,/Zach’s link is ready/);assert.equal(h.host.querySelectorAll('.lg-link-row button')[1].disabled,false);
 h.dom.window.close();
