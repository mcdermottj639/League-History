...
(function () {
  ...
  /* 🚨 A BOARD IS GAMES YOU CAN BET, NOT GAMES ESPN KNOWS ABOUT (v79).
     ...
     does not. */
  // Picks generated by the board include team codes in every market. Match
  // whole tokens, not substrings (e.g. LA must not match LAR). Existing picks
  // therefore reserve their game too, without a migration or new database rules.
  const mentionsTeam = (p, team) => String(p).toUpperCase().split(/[^A-Z0-9]+/).includes(String(team).toUpperCase());
  const matchesGame = (p, g) => mentionsTeam(p, g.a) || mentionsTeam(p, g.h);
  const gameTaken = (ow, g, me = LH && LH.me()) => syncLegs(ow).legs.some((leg) => leg.m !== me && matchesGame(leg.p, g));
  const playable = (ow) => gamesOf(ow).filter((g) => optionsFor(g).length && !gameTaken(ow, g));
  ...
  function selPick(ow) {
    ...
    if (!g || gameTaken(ow, g)) return null;
  ...
  function gameBoardHTML(ow) {
    ...
    return `<div class="lp-board">${games.map((g, i) => {
      ...
      if (!opts.length || gameTaken(ow, g)) return '';
  ...
  function pickHTML(ow) {
    ...
    return `${head}
      <div class="ffp-card lp-you">
        <p><b>One NFL bet, one pick per game.</b> ${games.length
          ? 'Tap a line below, or write your own prop.'
          : gamesOf(ow).some(g => optionsFor(g).length)
            ? 'Every posted matchup has been picked. A game returns if its owner clears or changes their pick.'
        ...
        <p class="lp-note">Picked matchups are removed for everyone else, including both teams and all markets. Include the team code when writing a prop.</p>
        ${noteFor('pick')}
        ${boardNote(ow)}
        ${gameBoardHTML(ow)}
        ${prop}
  ...
  document.addEventListener('click', async (e) => {
    ...
    if (act === 'save') {
      ...
      const me = LH.me();
      const chosen = selPick(ow);
      if (P.sel && !chosen) { P.sel = null; say('That game is already picked. Choose another matchup.', 'pick'); return render(); }
      ...
      const base = syncBase(P.file);
      // Re-read before saving: another member may have picked since this
      // phone opened the board. A failed check must not claim a reservation.
      P.saving = true;
      try {
        if (base) {
          const rows = await getPicks(base, ow.k);
          P.picks = { k: ow.k, rows: rows || {}, at: Date.now() };
          write(SYNC_KEY, P.picks);
        }
      } catch (e) {
        P.saving = false;
        say('Cannot check which games are taken. Try saving again when connected.', 'pick'); return render();
      }
      if (LH.me() !== me || !curOpen() || curOpen().k !== ow.k) { P.saving = false; return; }
      if (gamesOf(ow).some(g => matchesGame(p, g) && gameTaken(ow, g, me))) {
        P.saving = false; P.sel = null;
        say('That game is already picked. Choose another matchup.', 'pick'); return render();
      }
      /* After the availability check, keep the local draft before the write
         so a failed PUT does not lose it. A local save is not a reservation. */
      const at = Date.now();
      write(PICK_KEY, { k: ow.k, m: LH.me(), p, o, at });
      P.draftP = ''; P.draftO = ''; P.sel = null;
      if (!base) { P.saving = false; say('Saved. Send it to the chat so it makes the ticket.', 'pick'); return render(); }
      say('Saved. Putting it up for the others…', 'pick');
      render();
      putPick(base, ow.k, me, p, o, at).then(() => {
        P.saving = false;
