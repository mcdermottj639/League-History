...
(() => {'use strict';
  ...
  function gradePrediction(p){
    const source=S.season?.oracleResults,week=p.week||S.week,i=week-1;
    if(!source||!Number.isInteger(source.week)||week>=source.week||week<1)return null;
    const a=source.teams.find(t=>String(t.teamId)===String(p.matchup?.away?.id)),h=source.teams.find(t=>String(t.teamId)===String(p.matchup?.home?.id));
    if(!a||!h||String(a.schedule?.[i])!==String(h.teamId)||String(h.schedule?.[i])!==String(a.teamId))return null;
    const actual=[a.scores?.[i],h.scores?.[i]],outcomes=[a.outcomes?.[i],h.outcomes?.[i]];
    if(!actual.every(x=>typeof x==='number'&&Number.isFinite(x)))return null;
    const expected=actual[0]===actual[1]?['T','T']:actual[0]>actual[1]?['W','L']:['L','W'];
    if(!outcomes.every((x,j)=>x===expected[j]))return null;
    const chosen=p.winner===p.matchup.away.name?0:p.winner===p.matchup.home.name?1:null;
    const result=chosen===null?null:expected[0]==='T'?'tie':expected[chosen]==='W'?'right':'wrong';
    const predicted=[p.awayScore,p.homeScore];
    const errors=predicted.map((x,j)=>typeof x==='number'&&Number.isFinite(x)?Math.abs(x-actual[j]):null);
    return {actual,errors,result};
  }
  function records(w){
    const all=(S.data?.weeks||[]).filter(x=>x.published).flatMap(x=>(x.predictions||[]).map(p=>({...p,week:x.week})));
    const count=predictions=>predictions.reduce((o,p)=>{const g=gradePrediction(p);if(!g)return o;o.matchups++;if(g.result)o[g.result]++;for(const e of g.errors)if(e!==null){o.error+=e;o.scores++;}return o;},{right:0,wrong:0,tie:0,matchups:0,error:0,scores:0});
    return {week:count(w?.published?(w.predictions||[]).map(p=>({...p,week:w.week})):[]),all:count(all)};
  }
  const points=n=>Number(n.toFixed(2)).toString();
  const pickRecord=r=>r.right+r.wrong+r.tie?`${r.right}–${r.wrong}${r.tie?'–'+r.tie:''}`:'—';
  function accuracyHTML(r){return `<section class="or-accuracy" hidden aria-label="Score prediction accuracy"><h3>Score accuracy</h3><div class="or-accuracy-grid">${[['This week',r.week],['Season',r.all]].map(([label,x])=>`<div><small>${label}</small><strong>${x.scores?points(x.error/x.scores)+' pts':'—'}</strong><span>Average score error</span><small>${x.scores} team scores · ${x.matchups} final matchups</small></div>`).join('')}</div><p>Average distance from each team's final score. Lower is better. Only confirmed, completed weeks count; ties are shown separately in the pick record.</p></section>`;}
  function scoreResultHTML(p,week){const g=gradePrediction({...p,week});if(!g)return '<p class="or-result-pending" hidden>Score accuracy awaits confirmed final results.</p>';return `<section class="or-score-result" hidden><h3>Prediction vs final</h3><table><thead><tr><th>Team</th><th>Predicted</th><th>Final</th><th>Off by</th></tr></thead><tbody>${['away','home'].map((side,j)=>`<tr><th scope="row">${esc(p.matchup[side].name)}</th><td>${esc(p[side+'Score']??'—')}</td><td>${points(g.actual[j])}</td><td>${g.errors[j]===null?'—':points(g.errors[j])}</td></tr>`).join('')}</tbody></table><b>${g.result==='right'?'Correct winner':g.result==='wrong'?'Missed winner':g.result==='tie'?'Matchup tied':'Winner pick unavailable'}</b></section>`;}
  ...
  function displayPrediction(p,week){
    ...
    const side=(stored,key)=>{const live=byId[String(stored.id)],name=String(stored.name||live?.n||'Team'),entering=stored.record||(live?recordAt(live,week):'');return {...stored,name,enteringRecord:entering,record:p[key]||(p.winner?projectedRecord(entering,p.winner===name):entering)};};
  ...
  // Presentation-only: never rewrite stored prose, predictions, or published rows.
  function proseHTML(value){
    const lines=String(value||'No write-up yet.').split(/\r?\n/),first=lines[0]||'';
    const heading=/^.{1,180}\(\d+-\d+(?:-\d+)?\)\s+vs\.?\s+.{1,180}\(\d+-\d+(?:-\d+)?\)\s*$/i.test(first)?lines.shift():null;
    let end=lines.length-1;while(end>=0&&!lines[end].trim())end--;
    const verdict=end>=0&&/^.{1,80}\bwins?\s+\d+(?:\.\d+)?\s*[–—-]\s*\d+(?:\.\d+)?\.?\s*$/i.test(lines[end])?lines.splice(end,1)[0]:null;
    return `${heading?'<div class="or-prose-heading">'+esc(heading)+'</div>':''}<p>${esc(lines.join('\n'))}</p>${verdict?'<blockquote class="or-verdict">'+esc(verdict)+'</blockquote>':''}`;
  }
  function pickLabel(p){
    if(!p.winner)return 'Not selected';
    const a=p.awayScore,h=p.homeScore,chosen=p.winner===p.matchup.away.name?a:p.winner===p.matchup.home.name?h:null,other=p.winner===p.matchup.away.name?h:a;
    return typeof chosen==='number'&&typeof other==='number'&&Number.isFinite(chosen)&&Number.isFinite(other)&&chosen>other?p.winner+' by '+Number((chosen-other).toFixed(2)):p.winner;
  }
  function publicSide(p,side,week){
    const t=p.matchup[side],stored=t.enteringRecord||'—',manager=window.LeagueESPN?.mgrFor?.(t.name,[])||'',score=p[side+'Score'];
    return `<div class="or-side ${side==='home'?'or-side-home ':''}${p.winner===t.name?'or-side-picked':''}"><div class="or-team">${manager?logo(manager,60):'<span class="or-crest-fallback" aria-hidden="true">✦</span>'}<span class="or-team-name">${esc(t.name)}</span><span class="or-record-badge">${esc(stored)}</span><small>Entering Week ${week}</small></div><strong>${esc(score??'—')}<small>Projected score</small></strong><span class="or-projected-record">Projected record <b>${esc(t.record||'—')}</b></span></div>`;
  }
  function publicHTML(w,preview=false){
    if(!w?.published&&!preview)return '<section class="or-empty"><b>The next Hurdstradamus is waiting to be published.</b><p>Hurd’s prophecies will appear here for everyone once he releases the week.</p></section>';
    w={...w,predictions:(w.predictions||[]).map(p=>displayPrediction(p,w.week))};
    const r=records(w),notice=preview?'<p class="or-status"><b>Private preview</b> · Only Hurd can see this draft. <button data-or="edit">Back to editor</button></p>':'';
    const short=name=>String(name||'Team').split(/\s+/).map(x=>x[0]).join('').slice(0,4).toUpperCase();
    return `${notice}<section class="or-head or-editorial-hero"><div class="or-celestial" aria-hidden="true">✦</div><p class="or-kicker">The Commish Has Spoken</p><h2>Hurdstradamus</h2><span class="or-week-seal">Week ${w.week}</span><p class="or-published">${preview?'Draft preview — not published':'Published '+esc(new Date(w.publishedAt).toLocaleString())}</p><div class="or-head-stats" hidden><b>${pickRecord(r.week)}<small>This week · picks</small></b><b>${pickRecord(r.all)}<small>Season · picks</small></b></div></section>${preview?'':accuracyHTML(r)}
    <nav class="or-matchup-nav" aria-label="Jump to a prophecy"><span>${w.predictions.length} prophecies · Find your matchup</span><div>${w.predictions.map((p,i)=>`<button type="button" data-or-jump="${i}" aria-label="Game ${i+1}: ${esc(p.matchup.away.name)} versus ${esc(p.matchup.home.name)}"><b>${String(i+1).padStart(2,'0')}</b> ${esc(short(p.matchup.away.name))} · ${esc(short(p.matchup.home.name))}</button>`).join('')}</div></nav>
    ${w.predictions.map((p,i)=>`<article class="or-card or-editorial-card" id="oracle-game-${w.week}-${i}" tabindex="-1"><div class="or-card-kicker"><span>The Prophecy / ${String(i+1).padStart(2,'0')}</span><span>${p.upset?'Upset Watch':'Week '+w.week}</span></div><section class="or-matchup" aria-label="Projected matchup"><div class="or-versus">${publicSide(p,'away',w.week)}<b class="or-vs">VS</b>${publicSide(p,'home',w.week)}</div><div class="or-call"><span>Hurd’s pick</span><b>${esc(pickLabel(p))}</b></div></section>${preview?'':scoreResultHTML(p,w.week)}<section class="or-analysis"><b>Hurd’s breakdown</b>${proseHTML(p.writeup)}</section><footer><span>✧ The prophecy is set. Let the games decide.</span></footer></article>`).join('')}`;
  }
  function editorHTML(w){const all=matchupsFor(S.week),draft=Object.fromEntries((w?.predictions||[]).map(p=>[p.id,p])),complete=all.filter(m=>completePrediction(draft[m.id])).length;return `<section class="or-head"><p class="or-kicker">Commissioner mode · Week ${S.week}</p><h2>Craft the Prophecy</h2><p>${complete} of ${all.length} complete · drafts are private until published.</p><div class="or-progress"><i style="width:${all.length?complete/all.length*100:0}%"></i></div></section><p class="or-status" role="status">${esc(S.message)}</p>${all.map((m,i)=>{const p={matchup:m,...draft[m.id]};return `<article class="or-edit" data-or-id="${esc(m.id)}"><p class="or-edit-kicker">Matchup ${i+1} of ${all.length}</p><div class="or-edit-teams">${teamHTML(m.away)}<label>Projected score<input data-f="awayScore" type="number" step="any" inputmode="decimal" min="0" max="300" value="${esc(p.awayScore??'')}"></label>${teamHTML(m.home)}<label>Projected score<input data-f="homeScore" type="number" step="any" inputmode="decimal" min="0" max="300" value="${esc(p.homeScore??'')}"></label></div><div class="or-fields"><label>${esc(m.away.name)} projected record<input data-f="awayRecord" readonly aria-readonly="true" value="${esc(p.winner?projectedRecord(m.away.record,p.winner===m.away.name):m.away.record)}"></label><label>${esc(m.home.name)} projected record<input data-f="homeRecord" readonly aria-readonly="true" value="${esc(p.winner?projectedRecord(m.home.record,p.winner===m.home.name):m.home.record)}"></label></div><fieldset><legend>Predicted winner</legend>${[m.away.name,m.home.name].map(n=>`<label class="or-winner"><input data-f="winner" type="radio" name="${esc(m.id)}" value="${esc(n)}" ${p.winner===n?'checked':''}> ${esc(n)}</label>`).join('')}</fieldset><label>Hurd’s write-up<textarea data-f="writeup" maxlength="${WRITEUP_MAX}" placeholder="The crystal ball sees…">${esc(p.writeup??'')}</textarea><small data-count>0 / ${WRITEUP_MAX.toLocaleString()}</small></label><div class="or-fields"><label class="or-check"><input data-f="upset" type="checkbox" ${p.upset?'checked':''}> Upset Watch</label></div><p class="or-errors" hidden></p></article>`;}).join('')}<div class="or-actions"><button data-or="preview" ${S.busy?'disabled':''}>Preview</button><button data-or="save" ${S.busy?'disabled':''}>${S.busy?'Saving…':'Save draft'}</button><button data-or="publish" ${S.busy||all.length!==6||complete!==all.length?'disabled':''}>Publish Week ${S.week}</button></div><p class="or-lock" aria-live="polite"></p>`;}
